...
class WebsiteTemplateRepository {
    ...
    {
        $this->model = new RootWebsiteTemplate;
    ...
    public function get_list_datatable($post_data)
    {
        ...
        $query = RootWebsiteTemplate::select("*")->with(['admin','pivot_menus']);
        $total = $query->count();
    ...
    public function view_create()
    {
        $admin = Auth::guard('admin')->user();
        $menus = RootMenu::get();
        return view('admin.website.template.edit')->with(['operate'=>'create', 'encode_id'=>encode(0), 'menus'=>$menus]);
    }
    ...
    public function view_edit()
    {
        ...
        if($decode_id == 0)
        {
            return view('admin.website.template.edit')->with(['operate'=>'create', 'encode_id'=>$id]);
        }
        else
        {
            $menus = RootMenu::get();
            $mine = RootWebsiteTemplate::with([
                'menu',
            ])->find($decode_id);
            if($mine)
            {
                unset($mine->id);
                return view('admin.website.template.edit')->with(['operate'=>'edit', 'encode_id'=>$id, 'menus'=>$menus, 'data'=>$mine]);
            }
    ...
    public function save($post_data)
    {
        ...
        {
            $mine = new RootWebsiteTemplate;
            $post_data["admin_id"] = $admin->id;
        }
        else if($operate == 'edit') // 编辑
        {
            $mine = RootWebsiteTemplate::find($decode_id);
...
//                $mine->pivot_menus()->attach($post_data["menus"]);
                ...
                if(!empty($post_data["cover"]))
                {
                    ...
                    $result = $upload->upload($post_data["cover"], 'root-unique-templates' , 'cover_template_'.$encode_id);
                    if($result["status"])
                    {
...
//                // 保存位置
//                $qr_code_path = 'resource/root/unique/templates';
...
//            exit($e->getMessage());
    ...
    public function delete($post_data)
    {
        ...
        $mine = RootWebsiteTemplate::find($id);
        if($mine->admin_id != $admin->id) return response_error([],"你没有操作权限");
